...
BIgMAG (Board InteGrating Metagenome-Assembled Genomes) is a dashboard to display multiple metagenome quality metrics and taxonomical annotation from multiple tools using the unique file generated by [MAGFlow](https://github.com/jeffe107/MAGFlow).
...
### Practical tips
Even though both BIgMAG is able to handle as many samples as you wish, for visualization purposes it is recommended to have maximum between 15 and 20 samples (we run out of colors). In addition, the names of the samples should be quite short (between 5 and 10 characters) for them no to occupy all of the space in each plot.
